# Simulant Properties Guide

Identification data for gemstone simulants.

**Database integration:** All simulants with a database preset ID can be queried for full properties via `mineral-db --info <id>`. Properties shown below are for quick reference; the database is the authoritative source.

## Diamond Simulants

### Cubic Zirconia (CZ) {#cubic-zirconia}
**Database ID:** `cubic-zirconia`

| Property | Value | vs Diamond |
|----------|-------|------------|
| Composition | ZrO₂ | C |
| RI | 2.15-2.18 | 2.42 |
| SG | 5.6-6.0 | 3.52 |
| Hardness | 8-8.5 | 10 |
| Dispersion | 0.060 | 0.044 |
| Thermal conductivity | Low | Very high |
| Optic character | Isotropic | Isotropic |

**Detection**:
- Heavier (SG test)
- Lower thermal conductivity (diamond tester fails)
- Higher dispersion (rainbow fire)
- May show orange flash
- Warm to touch (vs diamond cold)

### Moissanite {#moissanite}
**Database ID:** `moissanite`

| Property | Value | vs Diamond |
|----------|-------|------------|
| Composition | SiC | C |
| RI | 2.648-2.691 | 2.42 |
| SG | 3.22 | 3.52 |
| Hardness | 9.25 | 10 |
| Dispersion | 0.104 | 0.044 |
| Birefringence | 0.043 | None |
| Thermal conductivity | High | Very high |

**Detection**:
- Doubling visible (birefringence)
- Extreme fire (high dispersion)
- May pass thermal tester
- Specific gravity lower
- Needle-like inclusions possible

### Synthetic Rutile {#synthetic-rutile}
**Database ID:** `synthetic-rutile`

| Property | Value | vs Diamond |
|----------|-------|------------|
| Composition | TiO₂ | C |
| RI | 2.616-2.903 | 2.42 |
| SG | 4.26 | 3.52 |
| Hardness | 6-6.5 | 10 |
| Dispersion | 0.280 | 0.044 |
| Birefringence | 0.287 | None |

**Detection**:
- Extreme dispersion (rainbow)
- Very strong doubling
- Much softer
- Yellow tint common
- Historical simulant (1940s-50s)

### YAG (Yttrium Aluminium Garnet) {#yag}
**Database ID:** `yag`

| Property | Value | vs Diamond |
|----------|-------|------------|
| Composition | Y₃Al₅O₁₂ | C |
| RI | 1.833 | 2.42 |
| SG | 4.55 | 3.52 |
| Hardness | 8.5 | 10 |
| Dispersion | 0.028 | 0.044 |

**Detection**:
- Lower RI (less brilliance)
- Lower dispersion (less fire)
- Historical simulant (1960s-70s)
- Largely replaced by CZ

### LuAG (Lutetium Aluminium Garnet) {#luag}
**Database ID:** `luag`

| Property | Value | vs Diamond |
|----------|-------|------------|
| Composition | Lu₃Al₅O₁₂ | C |
| RI | 1.842 | 2.42 |
| SG | 6.73 | 3.52 |
| Hardness | 8-8.5 | 10 |
| Dispersion | 0.031 | 0.044 |

**Detection**:
- Higher RI than YAG but still much lower than diamond
- Very high SG (noticeably heavy)
- Isotropic
- Less common than CZ or moissanite
- Sometimes used as coloured stone simulant

### Strontium Titanate (Fabulite)

| Property | Value | vs Diamond |
|----------|-------|------------|
| Composition | SrTiO₃ | C |
| RI | 2.409 | 2.42 |
| SG | 5.13 | 3.52 |
| Hardness | 5.5 | 10 |
| Dispersion | 0.190 | 0.044 |

**Detection**:
- Very soft (scratches easily)
- Extreme dispersion
- Historical simulant
- Rarely seen today

### GGG (Gadolinium Gallium Garnet) {#ggg}
**Database ID:** `ggg`

(Properties listed in main simulant tables above.)

### White Sapphire

| Property | Value | vs Diamond |
|----------|-------|------------|
| Composition | Al₂O₃ | C |
| RI | 1.762-1.770 | 2.42 |
| SG | 4.00 | 3.52 |
| Hardness | 9 | 10 |
| Dispersion | 0.018 | 0.044 |

**Detection**:
- Much lower RI (less brilliant)
- Much lower dispersion (no fire)
- Birefringent
- Natural material

### White Topaz

| Property | Value | vs Diamond |
|----------|-------|------------|
| Composition | Al₂SiO₄(F,OH)₂ | C |
| RI | 1.619-1.627 | 2.42 |
| SG | 3.53 | 3.52 |
| Hardness | 8 | 10 |
| Dispersion | 0.014 | 0.044 |

**Detection**:
- Very low RI (dull appearance)
- No fire
- Birefringent
- Perfect cleavage

## Ruby Simulants

### Red Glass

| Property | Value | vs Ruby |
|----------|-------|---------|
| RI | 1.45-1.70 | 1.762-1.770 |
| SG | 2.4-4.2 | 4.00 |
| Hardness | 5-6 | 9 |
| Optic character | Isotropic | Uniaxial |

**Detection**:
- Gas bubbles
- Swirl marks
- Lower RI
- Softer
- No pleochroism

### Red Spinel

| Property | Value | vs Ruby |
|----------|-------|---------|
| Composition | MgAl₂O₄ | Al₂O₃ |
| RI | 1.718 | 1.762-1.770 |
| SG | 3.60 | 4.00 |
| Hardness | 8 | 9 |
| Optic character | Isotropic | Uniaxial |

**Detection**:
- Lower RI
- Isotropic (no doubling)
- Different spectrum
- Historical confusion ("Balas ruby")

### Garnet (Pyrope/Almandine)

| Property | Pyrope | Almandine | vs Ruby |
|----------|--------|-----------|---------|
| RI | 1.714-1.742 | 1.790-1.830 | 1.762-1.770 |
| SG | 3.51-3.65 | 4.05-4.25 | 4.00 |
| Hardness | 7-7.5 | 7.5 | 9 |
| Optic | Isotropic | Isotropic | Uniaxial |

**Detection**:
- Isotropic
- Different spectrum (almandine)
- No fluorescence typically
- Different inclusions

### Rubellite Tourmaline

| Property | Value | vs Ruby |
|----------|-------|---------|
| RI | 1.624-1.644 | 1.762-1.770 |
| SG | 3.06 | 4.00 |
| Hardness | 7-7.5 | 9 |
| Pleochroism | Strong | Moderate |

**Detection**:
- Much lower RI
- Lower SG
- Different pleochroism
- 498nm absorption line

## Emerald Simulants

### Green Glass

| Property | Value | vs Emerald |
|----------|-------|------------|
| RI | 1.45-1.70 | 1.577-1.583 |
| SG | 2.4-4.2 | 2.72 |
| Hardness | 5-6 | 7.5-8 |
| Optic character | Isotropic | Uniaxial |

**Detection**:
- Isotropic
- Gas bubbles
- No Chelsea reaction
- No chromium spectrum

### Chrome Tourmaline

| Property | Value | vs Emerald |
|----------|-------|------------|
| RI | 1.624-1.644 | 1.577-1.583 |
| SG | 3.06 | 2.72 |
| Hardness | 7-7.5 | 7.5-8 |
| Birefringence | 0.018-0.020 | 0.005-0.009 |

**Detection**:
- Higher RI
- Higher birefringence
- 498nm absorption line
- Different inclusions

### Green Tsavorite

| Property | Value | vs Emerald |
|----------|-------|------------|
| RI | 1.739-1.744 | 1.577-1.583 |
| SG | 3.61 | 2.72 |
| Hardness | 7-7.5 | 7.5-8 |
| Optic character | Isotropic | Uniaxial |

**Detection**:
- Much higher RI
- Isotropic
- Higher SG
- No three-phase inclusions

### Chrome Diopside

| Property | Value | vs Emerald |
|----------|-------|------------|
| RI | 1.675-1.701 | 1.577-1.583 |
| SG | 3.29 | 2.72 |
| Hardness | 5.5-6 | 7.5-8 |

**Detection**:
- Higher RI
- Much softer
- Strong pleochroism
- Different spectrum

## Sapphire Simulants

### Blue Glass

| Property | Value | vs Sapphire |
|----------|-------|-------------|
| RI | 1.45-1.70 | 1.762-1.770 |
| SG | 2.4-4.2 | 4.00 |
| Hardness | 5-6 | 9 |

**Detection**:
- Isotropic
- Gas bubbles
- Lower RI
- Much softer

### Synthetic Blue Spinel

| Property | Value | vs Sapphire |
|----------|-------|-------------|
| RI | 1.727 | 1.762-1.770 |
| SG | 3.64 | 4.00 |
| Hardness | 8 | 9 |

**Detection**:
- Lower RI
- ADR in polariscope
- Red fluorescence (cobalt)
- Cobalt spectrum

### Tanzanite

| Property | Value | vs Sapphire |
|----------|-------|-------------|
| RI | 1.691-1.700 | 1.762-1.770 |
| SG | 3.35 | 4.00 |
| Hardness | 6-7 | 9 |

**Detection**:
- Lower RI
- Lower SG
- Strong trichroism
- Much softer

### Iolite

| Property | Value | vs Sapphire |
|----------|-------|-------------|
| RI | 1.542-1.551 | 1.762-1.770 |
| SG | 2.61 | 4.00 |
| Hardness | 7-7.5 | 9 |

**Detection**:
- Much lower RI
- Strong pleochroism (violet-blue-grey)
- Lower SG
- Softer

## Quick Detection Reference

### By Test

| Test | What It Reveals |
|------|-----------------|
| RI | Distinguishes most simulants |
| SG | Confirms identity |
| Polariscope | Optic character |
| Microscope | Inclusions, structure |
| UV fluorescence | Characteristic reactions |
| Spectrum | Chromophore identification |
| Thermal probe | Diamond testing |

### Common Confusion Pairs

| Genuine | Common Simulant | Key Difference | DB Query |
|---------|-----------------|----------------|----------|
| Diamond | CZ | Thermal conductivity | `mineral-db --info cubic-zirconia` |
| Diamond | Moissanite | Birefringence | `mineral-db --info moissanite` |
| Diamond | LuAG | RI, SG (very heavy) | `mineral-db --info luag` |
| Ruby | Garnet | Optic character | |
| Ruby | Red spinel | RI, optic character | |
| Emerald | Tsavorite | RI (much higher) | |
| Sapphire | Blue spinel | RI, fluorescence | |
| Alexandrite | CC sapphire | RI, SG | |
